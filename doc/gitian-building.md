...
*Setup instructions for a gitian build of Genzy Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Genzy Coin
...
These results are compared and only if they match, the build is accepted and provided for download.
...
- [Building Genzy Coin](#building-genzycoin)
...
Clone the git repositories for genzycoin and gitian and then checkout the genzycoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the genzycoin repository
...
To build Genzy Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the genzycoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/genzycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/GenzyCoin/GenzyCoin
...
For example:
```bash
URL=https://github.com/crowning-/genzycoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit genzycoin=${COMMIT} --url genzycoin=${URL} ../genzycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit genzycoin=${COMMIT} --url genzycoin=${URL} ../genzycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit genzycoin=${COMMIT} --url genzycoin=${URL} ../genzycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/genzycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/genzycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/genzycoin-build.assert
```
...
[GenzyCoin/gitian.sigs](https://github.com/GenzyCoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
