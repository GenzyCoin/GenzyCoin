...
public:
    ...
    bool WriteZGNZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGNZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGNZSeed_deprecated(uint256& seed);
    bool EraseZGNZSeed();
    bool EraseZGNZSeed_deprecated();

    bool WriteZGNZCount(const uint32_t& nCount);
    bool ReadZGNZCount(uint32_t& nCount);
