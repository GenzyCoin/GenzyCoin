...
CzGNZWallet::CzGNZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgnz seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGNZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGNZSeed_deprecated()) {
                    LogPrintf("%s: Updated zGNZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgnz seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgnz wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGNZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGNZCount(nCountLastUsed);
    else if (!walletdb.ReadZGNZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGNZWallet::Lock()
{
    seedMaster = 0;
}

void CzGNZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGNZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGNZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGNZWallet::LoadMintPoolFromDB()
{
...
void CzGNZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGNZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGNZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgnzTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGNZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGNZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgnzTracker which also adds to database
    pwalletMain->zgnzTracker->Add(dMint, true);
...
void CzGNZWallet::SeedToZGNZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGNZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGNZWallet::GenerateDeterministicZGNZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGNZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGNZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
