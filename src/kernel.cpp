...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZGnzStake(spend));
    } else {
        ...
        CGnzStake* gnzInput = new CGnzStake();
        gnzInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(gnzInput);
    }
