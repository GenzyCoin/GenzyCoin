...
#ifndef GENZYCOIN_ZGNZWALLET_H
#define GENZYCOIN_ZGNZWALLET_H
...
public:
    CzGNZWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGNZ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGNZ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GENZYCOIN_ZGNZWALLET_H
