...
CzGNZTracker::CzGNZTracker(std::string strWalletFile)
{
...
CzGNZTracker::~CzGNZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGNZTracker::Init()
{
...
bool CzGNZTracker::Archive(CMintMeta& meta)
{
...
bool CzGNZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGNZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGNZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGNZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzGNZTracker::GetSerialHashes()
{
...
CAmount CzGNZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGNZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGNZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGNZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGNZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGNZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGNZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGNZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGNZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGNZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGNZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGNZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGNZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGNZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGNZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGNZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGNZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzgnz from DB\n", __func__, listDeterministicDB.size());
    }
