...
#include "ui_zgnzcontroldialog.h"
...
std::set<std::string> ZGnzControlDialog::setSelectedMints;
std::set<CMintMeta> ZGnzControlDialog::setMints;

ZGnzControlDialog::ZGnzControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGnzControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGnzControlDialog::~ZGnzControlDialog()
{
    delete ui;
}

void ZGnzControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGnzControlDialog::updateList()
{
...
void ZGnzControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGnzControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGnz_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGnzControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGnzControlDialog::GetSelectedMints()
{
...
void ZGnzControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
