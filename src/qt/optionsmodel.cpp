...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GNZ);
    ...
    if (!settings.contains("nAnonymizeGenzyCoinAmount"))
        settings.setValue("nAnonymizeGenzyCoinAmount", 1000);

    nAnonymizeGenzyCoinAmount = settings.value("nAnonymizeGenzyCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGenzyCoinAmount"))
        SoftSetArg("-anonymizegenzycoinamount", settings.value("nAnonymizeGenzyCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in genzycoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGenzyCoinAmount:
            return QVariant(nAnonymizeGenzyCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGenzyCoinAmount:
            nAnonymizeGenzyCoinAmount = value.toInt();
            settings.setValue("nAnonymizeGenzyCoinAmount", nAnonymizeGenzyCoinAmount);
            emit anonymizeGenzyCoinAmountChanged(nAnonymizeGenzyCoinAmount);
            break;
        case CoinControlFeatures:
