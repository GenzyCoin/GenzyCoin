...
#include "config/genzycoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("genzycoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "genzycoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "genzycoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GNZ and zGNZ money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GNZ/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GNZ/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, genzycoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GNZ/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Genzy Coin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-gnzstake=<n>", strprintf(_("Enable or disable staking functionality for GNZ inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgnzstake=<n>", strprintf(_("Enable or disable staking functionality for zGNZ inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzgnz=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGNZ minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgnzbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zGNZ backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizegenzycoinamount=<n>     " + strprintf(_("Keep N GNZ anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Genzy Coin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("genzycoin-loadblk");
...
/** Sanity checks
 *  Ensure that Genzy Coin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Genzy Coin Core is shutting down."));
...
#endif
    // Make sure only a single Genzy Coin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Genzy Coin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Genzy Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    // Genzy Coin: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Genzy Coin specific: zerocoin and spork DB's
                ...
                // Genzy Coin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZGNZMinted();
                        RecalculateZGNZSpent();
                    }
                    RecalculateGNZSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Genzy Coin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Genzy Coin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Genzy Coin Core to complete") << "\n";
        ...
        zwalletMain = new CzGNZWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zGNZWallet
        uiInterface.InitMessage(_("Syncing zGNZ wallet..."));

        bool fEnableZGnzBackups = GetBoolArg("-backupzgnz", true);
        pwalletMain->setZGnzAutoBackups(fEnableZGnzBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zgnzTracker->Init();
...
//    }
//
//    nAnonymizeGenzyCoinAmount = GetArg("-anonymizegenzycoinamount", 0);
//    if (nAnonymizeGenzyCoinAmount > 999999) nAnonymizeGenzyCoinAmount = 999999;
//    if (nAnonymizeGenzyCoinAmount < 2) nAnonymizeGenzyCoinAmount = 2;
    ...
    LogPrintf("Anonymize Genzy Coin Amount %d\n", nAnonymizeGenzyCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1GNZ+1000 == (.1GNZ+100)*10
       10GNZ+10000 == (1GNZ+1000)*10
    */
